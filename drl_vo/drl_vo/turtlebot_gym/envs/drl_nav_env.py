...
class DRLNavEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self._cnn_data_sub = rospy.Subscriber("/cnn_data", CNNData, self._cnn_data_callback, queue_size=1, buff_size=2**24)
        ...
        self._final_goal_sub = rospy.Subscriber("/goal_pose", PoseStamped, self._final_goal_callback) #, queue_size=1)
        self._goal_status_sub = rospy.Subscriber("/navigate_to_pose/_action/status", GoalStatusArray, self._goal_status_callback) #, queue_size=1)
        ...
        # Publish goal:
        self._initial_goal_pub = rospy.Publisher('/goal_pose', PoseStamped, queue_size=1, latch=True)
        # Set model state:
    ...
    def _check_all_subscribers_ready(self):
        ...
        #self._check_subscriber_ready("/goal_pose", PoseStamped)
        self._check_subscriber_ready("/navigate_to_pose/_action/status", GoalStatusArray)
    ...
    def _goal_status_callback(self, goal_status_msg):
        ...
        if(len(goal_status_msg.status_list) > 0):
            ...
            if(last_element.status == 4): # succeeded 
